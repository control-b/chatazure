...
export function PhoenixProvider({ children }: PhoenixProviderProps) {
  ...
  const wsUrl = process.env.NEXT_PUBLIC_PHOENIX_WS_URL || 'ws://localhost:4000/socket'
  ...
  const joinRoom = (topic: string, params?: Record<string, any>) => {
    ...
    if (!ch) {
      ...
      ch.join().receive('ok', () => console.log(`[phoenix] joined ${topic}`))
                  .receive('error', (e: any) => console.error(`[phoenix] failed join ${topic}`, e))
                  .receive('timeout', () => console.error(`[phoenix] timeout join ${topic}`))
    }
    return ch
  }
