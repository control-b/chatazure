...
export function GeofencingPanel({
  ...
  const [eventFilter, setEventFilter] = useState<'all' | 'enter' | 'exit'>('all')
  ...
  const filteredEvents = geoEvents.filter(event => {
    switch (eventFilter) {
      case 'enter':
  ...
  const renderGeoEvent = (event: GeoEvent) => {
    ...
    return (
      <div key={event.id} className="p-3 border-b border-slate-100 last:border-b-0">
        <div className="flex items-start gap-3">
          <EventIcon className={cn("h-5 w-5 mt-0.5", eventTypeColors[event.type])} />
